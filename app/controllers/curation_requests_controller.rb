class CurationRequestsController < ApplicationController
    ...
    def status
      ...
      if (1..5).include?(@user_subscription.subscription_id)
        @curation_request_type = "delivery"
      else
        @curation_request_type = "shipment"
      end
      
      # if there is not an order placed, redirect to the new order page
      if @next_delivery.blank?
        redirect_to new_curation_request_path
      end
    ...
    def new
        ...
        if (1..5).include?(@user_subscription.subscription_id)
          @max_drink_number = 29
          @curation_request_type = "delivery"
          @curation_request_type_two = "delivered"
        else
          @max_drink_number = 23
          @curation_request_type = "shipment"
          @curation_request_type_two = "shipped"
        end
        # instantiate new order
        @curation_request = CurationRequest.new
        @delivery_preferences = DeliveryPreference.find_by_user_id(current_user.id)
        @max_large_format_drinks = 14
        @current_page = "orders"
        @form = "new"
        @curation_request.drink_option_id = @delivery_preferences.drink_option_id
        ...
        # set defaults
        @total_drinks = "TBD"
        @curation_request_estimate = "TBD"
        
    end # end of new method

    def create
        @curation_request = CurationRequest.new(order_params)
        # add additional attributes to order
        @curation_request.account_id = current_user.account_id
        @curation_request.user_id = current_user.id
        
        if !@curation_request.valid?
            @request_length = @curation_request.additional_requests.size
            if @curation_request.delivery_date.nil? || (@curation_request.number_of_beers == 0 && @curation_request.number_of_ciders == 0)
              flash[:failure] = "Please select the number of drinks and a delivery date"
            elsif @curation_request.delivery_date.nil?
              flash[:failure] = "Please select a delivery date"
            elsif @curation_request.number_of_beers == 0 && @curation_request.number_of_ciders == 0
              flash[:failure] = "Please select the number of drinks"
            else
              flash[:failure] = "Please limit the additional request to 500 characters"
            end
            render js: "window.location = '#{new_curation_request_path}'"
            return
        end

        # save order
        @curation_request.save

        @delivery_preferences = DeliveryPreference.where(user_id: current_user.id).first
        #@delivery_preferences.update(drinks_per_week: @curation_request.number_of_drinks, max_large_format: @curation_request.number_of_large_drinks, drinks_per_delivery: @curation_request.number_of_drinks)

        @new_delivery = Delivery.create(account_id: current_user.account_id,
                                order_id: @curation_request.id,
                                delivery_date: @curation_request.delivery_date,
                                status: "admin prep",
                                subtotal: 0,
                                sales_tax: 0,
                                total_drink_price: 0,
        ...
        # notify admin               
        AdminMailer.admin_customer_order(current_user, @curation_request, "new").deliver_now
        
        # set redirect
        redirect_to curation_request_status_path
    ...
    def edit
      ...
      # get order info
      @curation_request = CurationRequest.find_by_id(params[:id])
      @curation_request_date = DateTime.parse(@curation_request.delivery_date.to_s)
      # get delivery info
      ...
      # set drink category choice
        ...
        if @delivery_preferences.beer_chosen
            ...
            @number_chosen = @number_chosen + 1
            @drink_categories << "beer"
            @number_of_beers = @curation_request.number_of_beers
        end
        if @delivery_preferences.cider_chosen
            @cider_chosen = true
            @number_chosen = @number_chosen + 1
            @drink_categories << "cider"
            @number_of_ciders = @curation_request.number_of_ciders
        end
        #if @delivery_preferences.wine_chosen
        #    @wine_chosen = true
        #    @number_chosen = @number_chosen + 1
    ...
    def update
      @curation_request = CurationRequest.update(params[:id],order_params)
      # add additional attributes to order
      @curation_request.account_id = current_user.account_id
      @curation_request.user_id = current_user.id
      ...
      @related_delivery = Delivery.find_by_order_id(@curation_request.id)
      @related_delivery.update(delivery_date: @curation_request.delivery_date)
    ...
    def process_ad_hoc_approval
      ...
      @curation_request = CurationRequest.find_by_id(@next_delivery.order_id)
      ...
      # notify admin               
      AdminMailer.admin_customer_order(current_user, @curation_request, "approved").deliver_now
