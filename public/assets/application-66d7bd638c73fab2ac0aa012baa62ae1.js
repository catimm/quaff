...
$(function() {
	...
	var word_array = [
	 ...
	 if (gon.beer_array != undefined) {
		 // create word cloud and return to view
	     for (i = 0; i < gon.beer_array.length; i++) { 
		    if ( gon.beer_array[i][1].length > 0 ) {
			    $("#beer_"+gon.beer_array[i][0]).jQCloud(gon.beer_array[i][1], {
			  		autoResize: true
			     });
			 } else {
				$("#beer_"+gon.beer_array[i][0]).jQCloud(word_array, {
			  		autoResize: true
			     });
			 }
		 }
...
$(function() {
	...
	$("#rater").on("slide", function(slideEvt) {
		if (slideEvt.value == '-0.5') {
			...
			$("#visible-rating h1").css({"font-size":"82px","font-weight":"500","color":"#0087cb","margin":"0"});
		} else {
